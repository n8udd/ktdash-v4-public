...
const CACHE_TIMES = {
  // Cache these endpoints for 6 hours
  LONG: 6 * 60 * 60 * 1000,
...
const CACHE_CONFIG = {
  ...
  patterns: [
    {
      // Match API killteam detail (with ID)
      match: /^\/api\/killteams\/[^\/]+$/,
      maxAge: CACHE_TIMES.SHORT,
      description: 'API killteam detail'
    },
    {
      // Match killteam detail page (with ID)
      match: /^\/killteams\/[^\/]+$/,
      maxAge: CACHE_TIMES.SHORT,
      description: 'Killteam detail page'
    },
    {
      // Match images
      match: /^\/img\/[^\/]+$/,
